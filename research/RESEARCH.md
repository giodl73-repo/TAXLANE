# TAXLANE Research — Tracks and Paper Inventory

Each **track** is a budget area; each produces a panel-reviewed paper arguing the
fair rate (cut or increase), the needs, and the modernization case. Tracks draw on
the completed design and operating-system waves for sourced figures.

## Tracks

| # | Track (budget area) | Paper slug | Core thesis | Direction |
|--:|---|---|---|---|
...
| 2 | The Old-Age Tax and the Cap | T2 | panel | 1 | 2.57 | (markdown) |
...
1. `plan.md` per paper → draft `paper.md`.
2. Panel review round (7 reviewers from `REVIEWERS.md`) → `reviews/REVIEW-*.md` + `SYNTHESIS.md`.
3. `REVISION-PLAN.md` → revise → next round, until the acceptance gate clears.
4. Optional promotion to LaTeX/PDF for a formal publication set.

## Build order

T1 (health — strongest evidence) first as the template, then T5 (revenue, the
cross-cutting frame), then T2–T4, then the T0 synthesis once the tracks land.

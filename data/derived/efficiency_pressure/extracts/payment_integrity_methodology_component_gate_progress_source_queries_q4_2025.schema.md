# Payment Integrity Methodology Component Gate Progress Source Queries Schema

One JSON object per line.

| Field | Type | Meaning |
|---|---|---|
| `record_id` | string | Stable component gate progress source-query identifier. |
| `record_family` | string | Must be `payment_integrity_methodology_component_gate_progress_source_query`. |
| `source_component_gate_progress_source_target_record_id` | string | Source target that created this query. |
...
| `next_artifact_family` | string | Must be `payment_integrity_methodology_component_gate_progress_source_query_run`. |

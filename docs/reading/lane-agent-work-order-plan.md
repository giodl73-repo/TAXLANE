# Lane agent work-order plan

Record:
`data/derived/breadth_benchmark_matrix/lane_agent_work_order_plan.v1.draft.json`

Yes, agents can scale the lane-depth work. The safe pattern is one lane per agent, one clean worktree per lane, and one integration review after each wave.

Each lane agent gets the same job:

- read the governing contracts and the lane-depth tracker;
- produce a lane depth card or overlay card;
- produce a schema, public reader, blocker list, validator, focused test, and pulse note;
- answer the same public questions for that lane;
- leave missing values null and blocked claims false.

The wave order is:

1. Health, Social Security, and Defense.
2. Income security/family, Education/workforce, and Veterans.
3. Disaster, Justice, and Science/energy/environment.

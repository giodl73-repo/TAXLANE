...
added together. The original assessment baseline is commit
`81b7c4acd2d1e4f785c1cdb529e1da283b1f5eca`; the 2026-08-01 NET rerun
incorporates the later admitted CORE-G and reduced-form feedback evidence. It
does not score the underlying federal systems.
...
| NET | endogenous accounting result | 2 | 2 | 2 | 2 | 3 | 1 | 2 | 14/21 | 1 |
...
The repo now admits the hash-custodied CORE-G FY2025–FY2035 annual CBO debt,
net-interest, and other-financing path. Its deterministic validator replays all
eleven topline and debt identities with a zero policy delta. The matching CBO
...
an unknown holder allocation with reproducible bounds. The empirical marginal-
rollover feature raises Iteration and NET to 14/21 because the observed term
mix now drives an executable monthly refinance loop with invariant tests.
...
holder intervals are now ready. Twelve MSPD snapshots also supply an inclusive
434-row issuance/reopening population, empirical term mix, observed sensitivity
rails, and principal-preserving marginal rollover. Future total issuance and
bucket-rate and stress paths remain for a full-stock mode. OMB's gross-to-net
components reconcile, while matching CBO-vintage component values remain
unavailable.
NET can accept a future admitted primary delta through the bounded reduced-form
and marginal rollover engines; a full-stock result must wait for the remaining
forecast and rate inputs.
...
- **Source Custodian:** admitted annual inputs are hash-custodied and source-
  labeled; incompatible maturity and bucket-stress inputs remain null.
...
- **Fiscal Sustainability / Reform Skeptic:** no candidate, savings, solver,
  balanced-budget, or public-release gate changes; only NET's tested repository
  capability advances.
...
bridge. This assessment admits the annual current-law debt context but changes
no target cost, allocation, savings amount, endogenous debt-interest result,
solver result, rate, public card, or release status.

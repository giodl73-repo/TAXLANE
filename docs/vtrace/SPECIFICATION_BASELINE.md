...
| SPEC-TAX-004 | REQ-TAX-004 | accountability | target | Accountability records will distinguish performance indicators, anomaly indicators, source evidence, review status, and allegation boundaries. | schema/research inspection | source-custody review | Source Custodian | high | planned |
...
| SPEC-TAX-006 | REQ-TAX-006 | architecture | target | Rust workspace will add a domain crate before fraud/waste accountability checks are encoded beyond documentation. | architecture review | cargo test | Maintainer | medium | planned |
